...
type Props = {
  assetMap?: {
    ...
    baseUrl: string,
    initialI18nStore?: object,//to be used with the middleware
...
const AppWithNavDemo: React.FC<Props> = ({ assetMap }) => {
  ...
  const additionStyles = () => {
    if (assetMap) {
      const _additionStyles = assetMap['additional-styles'].map(
        (additionalStyle) => {
  ...
  const additionalJss = () => {
    if (assetMap) {
      const _additionalJss = assetMap['additional-jss'].map(
        (additionalJs) => {
